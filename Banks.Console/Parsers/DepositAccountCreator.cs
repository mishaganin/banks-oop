...
public class DepositAccountCreator : AbsParser
{
    public override string Execute(string command)
    {
        if (command == "create deposit account")
        {
            System.Console.WriteLine("Set data about deposit account:");
            ...
            Guid accountId = CentralBank.GetConnection().GetBankById(new Guid(bankId))
                                        .CreateDepositAccount(balance, new Guid(clientId)).Id;
            System.Console.WriteLine($"Deposit account's id: {accountId}");
            return "Deposit account successfully created!";
        }
