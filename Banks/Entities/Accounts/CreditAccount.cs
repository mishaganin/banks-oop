...
public class CreditAccount : IAccount, IAccrueObserver
{
    ...
    private List<decimal> _monthlyDeposits;
    private int _daysUntilNextMonth;
    public CreditAccount(Bank bank, decimal balance, decimal percent, decimal limit, decimal fixedCommission, Client client, bool isQuestionable)
    {
        Bank = bank;
        Balance = balance;
        Percent = percent / 100;
        ...
        _monthlyDeposits = new List<decimal>();
    ...
    public decimal Replenish(decimal amount)
    {
        ...
        if (totalAmount < 0 || (Balance + amount > Limit))
        {
            throw new Exception();
        }

        Balance += totalAmount;
        Bank.IncreaseCapital(totalAmount);
        CreateTransaction(_types[1], totalAmount, this);
    ...
    {
        foreach (decimal deposit in _monthlyDeposits)
        {
            Balance += deposit;
    ...
    {
        _monthlyDeposits.Add(Accruals);
        Accruals = 0;
    }

    private void DailyCommission(int days)
    {
        if (Balance < 0)
        {
            Commission += FixedCommission * days;
        }
    }
